...
function get_setters(element2) {
  var setters = [];
  var descriptors = get_descriptors(element2.__proto__);
  ...
  for (const key in $$props) {
    ...
    if (setter) {
      if (exports.includes(key)) {
        throw new Error(
          `Component ${component2.filename} has an export named ${key} that a consumer component is trying to access using bind:${key}, which is disallowed. Instead, use bind:this (e.g. <${name} bind:this={component} />) and then access the property on the bound component instance (e.g. component.${key}).`
        );
      }
      if (!bindable.includes(key)) {
        throw new Error(
          `A component is binding to property ${key} of ${name}.svelte (i.e. <${name} bind:${key} />). This is disallowed because the property was not declared as bindable inside ${component2.filename}. To mark a property as bindable, use the $bindable() rune in ${name}.svelte like this: \`let { ${key} = $bindable() } = $props()\``
        );
      }
    }
  }
...
//# sourceMappingURL=chunk-SVTV3IA4.js.map
