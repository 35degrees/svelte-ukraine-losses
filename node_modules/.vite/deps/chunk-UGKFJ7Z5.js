...
function validate_effect(effect2, rune) {
  if (effect2 === null) {
    throw new Error(
      "ERR_SVELTE_ORPHAN_EFFECT" + (DEV ? `: ${rune} can only be used inside an effect (e.g. during component initialisation)` : "")
    );
  }
  if (is_destroying_effect) {
    throw new Error(
      "ERR_SVELTE_EFFECT_IN_TEARDOWN" + (DEV ? `: ${rune} cannot be used inside an effect cleanup function.` : "")
    );
  }
}
  ...
  if (typeof value === "object" && value != null && !is_frozen(value)) {
    ...
    if (prototype === object_prototype || prototype === array_prototype) {
      ...
      if (DEV) {
        ...
        value[STATE_SYMBOL].owners = parent === null ? current_component_context !== null ? (
          // @ts-expect-error
    ...
    try {
      ...
      current_target2.disabled) {
        if (is_array(delegated)) {
          ...
          fn.apply(current_target2, [event2, ...data]);
        } else {
          delegated.call(current_target2, event2);
        }
      }
    } finally {
      if (!event2.cancelBubble && parent_element !== handler_element && parent_element !== null && current_target2 !== handler_element) {
        next(parent_element);
      }
    }
  }
...
//# sourceMappingURL=chunk-UGKFJ7Z5.js.map
